...
var Y = {}, oe = { exports: {} }, ie = { exports: {} }, re, $e;
function Xt() {
  if ($e) return re;
  $e = 1;
  const b = ee, y = 1800;
...
function q() {
  return We || (We = 1, (function(b) {
    const y = ee, k = Ct, f = Ue, p = ae(), n = jt, r = te, o = It, m = 300 * 1e3, e = 30 * 1e3, l = 1e3;
    let c = 0;
    ...
    b.exports.resolveHostname = (a, h) => {
      ...
      for (let I = 0; I < E.length; I++) {
        const T = E[I], j = T.indexOf("=");
        if (j > 0) {
          const M = T.substring(0, j).trim(), L = T.substring(j + 1).trim();
          M && (A[M] = L);
        }
      }
      let C;
    ...
    function d(a) {
      ...
      let _ = (E, S, A, ...C) => {
        let I = "";
        S && (S.tnx === "server" ? I = "S: " : S.tnx === "client" && (I = "C: "), S.sid && (I = "[" + S.sid + "] " + I), S.cid && (I = "[#" + S.cid + "] " + I)), A = k.format(A, ...C), A.split(/\r?\n/).forEach((T) => {
          console.log("[%s] %s %s", (/* @__PURE__ */ new Date()).toISOString().substr(0, 19).replace(/T/, " "), u.get(E), I + T);
        });
      }, w = {};
      return a.forEach((E) => {
        w[E] = _.bind(null, E);
...
function Ot() {
  ...
  const h = (T) => String.fromCodePoint(...T), u = function(T) {
    ...
    return T + 22 + 75 * (T < 26) - ((j != 0) << 5);
  }, w = function(T, j, M) {
    let L = 0;
    for (
      T = M ? g(T / n) : T >> 1, T += g(T / j);
      /* no initialization */
      T > x * f >> 1;
      L += y
    )
    ...
    R < 0 && (R = 0);
    for (let z = 0; z < R; ++z)
      T.charCodeAt(z) >= 128 && t("not-basic"), j.push(T.charCodeAt(z));
...
function Be() {
  if (st) return ve;
  st = 1;
  const b = K, y = Ue, k = Ot(), f = P.PassThrough, p = q(), n = ne(), r = Ht(), o = Nt(), m = ei(), e = ae(), l = F(), c = ti(), s = qt(), x = ii();
  class g {
    constructor(t, i) {
...
`), I.stream(t, i, (T) => {
        ...
        if (this.content) {
          ...
          if (this.content._resolve) {
            let C = [], I = 0, T = !1, j = this._getStream(this.content);
            j.on("error", (M) => {
              T || (T = !0, w(M));
            }), j.on("readable", () => {
              let M;
              ...
              T || (T = !0, this.content._resolve = !1, this.content._resolvedValue = Buffer.concat(C, I), setImmediate(A));
            });
          } else
            setImmediate(A);
          return;
...
function ri() {
  if (pt) return Ee;
  pt = 1;
  const b = ai(), y = ni(), k = oi(), f = P.PassThrough, p = Ue, n = At, r = K, o = "sha256", m = 2 * 1024 * 1024;
  class e {
    constructor(s, x, g, v) {
...
function ci() {
  if (dt) return Te;
  dt = 1;
  const b = V, y = q(), k = Lt(), f = si(), p = ri(), n = pi(), r = F(), o = Ct, m = ee, e = D, l = li(), c = te, s = jt, x = K;
  class g extends b {
    constructor(t, i, d) {
        ...
        switch (h) {
          ...
          case "socks4a": {
            ...
            let _ = (w) => {
              ...
              if (i.auth) {
                let I = decodeURIComponent(i.auth.split(":").shift()), T = decodeURIComponent(i.auth.split(":").pop());
                E ? (C.proxy.userId = I, C.proxy.password = T) : A === 4 ? C.userid = I : C.authentication = {
                  username: I,
                  password: T
                };
              }
              S.createConnection(C, (I, T) => I ? a(I) : a(null, {
...
function De() {
  if (ht) return Ae;
  ht = 1;
  const b = D, y = V.EventEmitter, k = te, f = Mt, p = It, n = K, r = di(), o = P.PassThrough, m = q(), e = 120 * 1e3, l = 600 * 1e3, c = 30 * 1e3, s = 30 * 1e3, x = () => {
  };
  class g extends y {
    constructor(t) {
    ...
    login(t, i) {
      ...
      if (this.customAuth.has(this._authMethod)) {
        let a = this.customAuth.get(this._authMethod), h, u = !1, _ = () => {
          ...
          sendCommand: (S, A) => {
            let C;
            return A || (C = new Promise((I, T) => {
              A = m.callbackPromise(I, T);
            })), this._responseActions.push((I) => {
              h = I;
              let T = I.match(/^(\d+)(?:\s(\d+\.\d+\.\d+))?\s/), j = {
                command: S,
                response: I
              };
              T ? (j.status = Number(T[1]) || 0, T[2] && (j.code = T[2]), j.text = I.substr(T[0].length)) : (j.text = I, j.status = 0), A(null, j);
...
function gs() {
  if (gt) return Me;
  gt = 1;
  const b = V, y = mi(), k = De(), f = zt(), p = q(), n = F(), r = D;
  class o extends b {
    constructor(e) {
...
function vs() {
  if (vt) return Le;
  vt = 1;
  const b = V, y = De(), k = zt(), f = q(), p = Rt(), n = F(), r = D;
  class o extends b {
    constructor(e) {
      super(), e = e || {}, typeof e == "string" && (e = {
        url: e
...
var Ss = ys();
const Ts = /* @__PURE__ */ Vt(Ss), ks = Bt(import.meta.url), St = Ut(ks), Pt = process.env.VITE_DEV_SERVER_URL !== void 0, Pe = ze(Z.getPath("userData"), "window-state.json");
Pt && (process.env.ELECTRON_DISABLE_SECURITY_WARNINGS = "true");
let H;
function As() {
  try {
    if (Dt(Pe))
      return JSON.parse(Ft(Pe, "utf-8"));
  } catch (b) {
