const SettingCard = ({ title, description, icon: Icon, children }) => {
  return (
    <section className="rounded-2xl border border-[var(--c-border)] bg-[var(--c-surface)] p-4 shadow-[0_14px_30px_-24px_rgba(15,23,42,0.7)] sm:p-5">
      <div className="flex items-center gap-3">
        {Icon && (
          <div className="flex h-10 w-10 items-center justify-center rounded-xl bg-[var(--c-panel)] text-[var(--c-accent)] shadow-[0_4px_12px_-4px_rgba(0,0,0,0.5)]">
            <Icon className="h-5 w-5" />
          </div>
        )}
        <h2 className="font-title text-xl font-bold text-[var(--c-text)]">{title}</h2>
      </div>
      <p className="mt-2 text-sm font-medium text-[var(--c-muted)]">{description}</p>
      <div className="mt-4">{children}</div>
    </section>
  );
};

export default SettingCard;
