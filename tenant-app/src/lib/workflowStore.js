...
export const createProforma = async (tenantId, proformaId, payload) => {
  try {
    ...
    await setDoc(ref, {
      ...payload,
      amountPaid: 0,
      balanceDue: Number(payload.totalAmount || 0),
...
export const upsertTracking = async (tenantId, trackingId, payload) => {
  try {
    ...
    // Validate bounds for trackings array (max 36)
    if (Array.isArray(payload.trackingNumbers) && payload.trackingNumbers.length > 36) {
       payload.trackingNumbers = payload.trackingNumbers.slice(0, 36);
    }
