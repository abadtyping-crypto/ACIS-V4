...
const ClientPaymentSection = ({ isOpen, onToggle, refreshKey }) => {
    ...
    const onSave = async (e) => {
        ...
        try {
            ...
            let smsMsg = 'SMS not sent.';
            if (selectedClient?.primaryMobile) {
                const shouldSendSms = window.confirm('Send SMS acknowledgement to client mobile now?');
                if (shouldSendSms) {
                    ...
                    if (smsRes.ok && !smsRes.skipped) smsMsg = 'SMS sent.';
                    else if (smsRes.ok && smsRes.skipped) smsMsg = `SMS skipped (${smsRes.reason || 'disabled'}).`;
                    else smsMsg = `SMS failed: ${smsRes.error}`;
                }
            }

            setStatus({ message: `Payment saved (${displayTxId}). ${mailMsg} ${smsMsg}`, type: 'success' });
            setForm({
                date: new Date().toISOString().slice(0, 10),
                clientSearch: '',
                clientId: '',
